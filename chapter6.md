...
*** =instructions
Buatlah sebuah `list` bernama `my_list` berisi variabel `my_vector`, `my_matrix`, dan` my_df` sebagai komponennya.
...
*** =hint
`shining_list <- list(moviename = mov)` adalah sebagian jawabannya; tugas Anda yaitu menambahkan `act` dan` rev` ke dalam `list` dengan nama yang sesuai.
